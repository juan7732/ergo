...
> A `winget` package (`juan7732.ergo`, portable, command alias `ergo`) is
> generated by the release pipeline and submitted to `microsoft/winget-pkgs`
> on every release. The first submission is pending winget-pkgs moderation;
> `winget install juan7732.ergo` works once it merges
> ([#6](https://github.com/juan7732/ergo/issues/6) tracks it).
