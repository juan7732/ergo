...
**Task runner:** Use `just` for build/test/lint. Run `just --list` to see all recipes.
Run `just check` before committing. Run `just test-pkg <path>` for focused testing.
...
**Go Quirks in VSCode** When creating new .go files, the package declaration line will be auto-generated by the Go extension on save. Do not include it in file creation if it will cause a duplicate.
